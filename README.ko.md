...
**v0.6.0 — 베타.** 이 저장소는 계속 개발 중이며, 인터페이스·동작은 업데이트마다 바뀔 수 있다.
...
`emucap-ppsspp-bridge`(PSP launch helper). Source build의 의존성은 전부 crates.io이고
...
재실행하기 전에 `status.continuity`·`status.runtime_instance`·`get_failure_context`를 확인한다.
...
- **Mesen2 (SNES·Game Gear·Game Boy·GBC·GBA·NES)** — Mesen2 앱 설치 + 최초 1회 GUI에서 Script
  window의 network access 허용. 어댑터 Lua가 동봉돼 있어 빌드가 없다. 시스템별 엔트리로 처리한다
  (SNES는 65816, Game Gear/Master System은 Z80, Game Boy/GBC는 SM83, GBA는 ARM7, NES는 6502). GBA는
  실 BIOS(`gba_bios.bin`, 비커밋)가 필요하고 SNES/Game Gear/GB/GBC/NES는 필요 없다. → `adapters/mesen2/README.md`
- **Mednafen (Saturn·PSX·PCE·MD·WonderSwan/WSC)** — `adapters/mednafen/build.sh`로 포크를 빌드한다(SDL 필요:
  macOS `brew install sdl2`, Linux `libsdl2-dev`). 한 바이너리가 다섯 시스템을 모두 처리한다.
...
- 바이너리: `emucap`(케이스 번들 `finalize`/`inspect`), `emucap-mcp`(제어 MCP — 실행 중 에뮬레이터
  ...
  `emucap-broker`(다중 세션 연결 공유), 그리고 빌드 절에 적은 PC-98/NDS/PSP launch bridge.
