...
// 다른 어댑터·status.memory_types와 일치하는 supported 목록(honest 에러 message에 동반).
...
// honest 구조화 에러(균일 인터페이스): 미지원 memory_type은 silent-accept 말고 supported를 동반해
...
void rearm_breakpoints() {
	g_bp_addrs.clear();
	for (const auto& b : g_bps) g_bp_addrs.insert(b.addr);
...
bool emucap_exec_bp_check(uint32_t pc) { return g_bp_addrs.find(pc) != g_bp_addrs.end(); }
