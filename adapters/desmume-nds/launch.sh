...
if [ "$OLD_BRIDGE_ALIVE" = "1" ]; then
  ...
  if [ "$OLD_EMU_ALIVE" = "0" ] \
     ...
     && printf '%s\n' "$OLD_BRIDGE_COMMAND" | grep -E "127\\.0\\.0\\.1:${ARM9_PORT}([[:space:]]|$)" >/dev/null \
     && printf '%s\n' "$OLD_BRIDGE_COMMAND" | grep -E "127\\.0\\.0\\.1:${ARM7_PORT}([[:space:]]|$)" >/dev/null; then
...
if [ "$OLD_EMU_ALIVE" = "1" ]; then
  echo "ERROR: recorded DeSmuME PID $OLD_EMU is still live; use the MCP replace path or stop that exact generation first." >&2
  exit 3
fi
if command -v lsof >/dev/null 2>&1; then
...
BRIDGE_PID="$(EMUCAP_NAME="$NAME" EMUCAP_SESSION_TOKEN="$SESSION_TOKEN" EMUCAP_CONTENT="$ROM" EMUCAP_BUILD_HASH="$EMUCAP_BUILD_HASH" EMUCAP_EMULATOR_PID="$EMU_PID" \
