...
try {
  ...
  if (-not $luaDirectory.Equals($adapterDirectory, [System.StringComparison]::OrdinalIgnoreCase)) {
    ...
    $dirtyFiles = @(& git -C $here status --porcelain -- emucap-core.lua emucap_deferred.lua emucap_dump.lua emucap_freeze_state.lua emucap_native_callstack.lua emucap_memory.lua emucap_step.lua emucap_tx.lua emucap_state_io.lua emucap_snapshot.lua emucap_recording.lua $entryName 2>$null)
