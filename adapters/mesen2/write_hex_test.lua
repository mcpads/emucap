...
local function parse_write_hex(hex)
  if type(hex) ~= "string" or #hex < 2 or #hex % 2 ~= 0 then return nil, "bad_params" end
  local bytes = {}
  for i = 1, #hex, 2 do
...
-- Empty writes are rejected to match the MCP contract.
eq(parse_write_hex(""), nil, "empty rejected")
