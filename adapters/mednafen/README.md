...
- **Supported (added from previously unsupported)**: `dump_memory` (dedicated RAM/VRAM with no cap, synthetic full-bus with a 64MB cap-skip),
  ...
  layout-aware · path-independent, but not a track-byte identity) + sha1 (entry file)). The Control MCP
  adds the launch-bound SHA-256 `content_identity` for CUE graphs and uses it as the opaque Tracking ID.
