...
[Starting and launching]
  ...
  5. To end a managed emulator, call `stop(launch_id)` with the exact `status.runtime_instance.launch_id`. `stop` is a host lifecycle tool, not an adapter method, so it is not listed in `status.methods`.

`display:true` opens a native HITL window only on adapters that support it. Mednafen audio is disabled by default and is enabled independently with `sound:true`; unsupported adapters reject that option. Never create an ad hoc `nohup` launch and never terminate by broad executable name. `stop` verifies the current generation, lease, and process-start identities, waits for terminal process states, and preserves failure evidence. It refuses broker-owned, unmanaged, mismatched, or unverifiable processes instead of guessing ownership.
...
Do not edit runtime files to clear a lease or change generation identity. Use `launch(..., replace:true)` only for an intentional replacement; it fails closed unless PID and process-start identity can be verified. Use `stop(launch_id)` for explicit termination even when the adapter transport is disconnected; a stale launch ID or non-held lease must fail without signalling a process. `get_failure_context` reads the last good state and host or adapter failure without issuing a new emulator request. When Flycast advertises `dismiss_failure`, inspect the quarantined state first and call it only when investigation is complete; it continues the existing termination path and does not repair the game.
