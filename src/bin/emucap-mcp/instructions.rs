...
[입력] 버튼명은 status.input_buttons, 동사 가용성은 status.methods가 정본(예: press_buttons 미advertise면 set_input+tap). 입력 동사 3부류: set_input(buttons)=패드 상태 설정(유지). press_buttons(buttons, frames)=N프레임 *실시간* 누름·뗌 — **frozen이면 자동 resume(freeze 풀림); 길게 누르면 auto-repeat**. **frozen 유지 결정론 입력은 freeze-tap**: tap(buttons, press_frames=2, after_frames=0)=auto-repeat 없이 1탭(after_frames>0=떼고 그만큼 진행, 호출 후 frozen 유지), tap_sequence(steps)=여러 탭 한 콜(메뉴 네비), hold_until(buttons, memory_type, address, length, max_frames)=watch 메모리 변할 때까지 frozen hold 후 뗌(반환 {changed,frames,before,after}). 즉 frozen 결정론 1칸=tap, 실시간=press_buttons. 터치스크린(NDS)은 touch(x, y[, frames])로 하단 화면 좌표를 누른다 — frames=탭, 생략=hold, frozen에선 frames 거부(resume 후 실시간). 멀티코어(NDS ARM9/ARM7)는 get_state/resume/pause/step에 cpu 인자(생략=기본 코어).
