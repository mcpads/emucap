...
use emucap::neogeo_bridge::NeoGeoBridge;
...
fn main() -> anyhow::Result<()> {
    ...
    let system = &args[2];
    let (gdb_host, gdb_port) = if let Some(raw) = args.get(3) {
        parse_endpoint(raw)?
    } else {
        ("127.0.0.1".into(), 3265)
    };
    ...
    eprintln!(
        "[mame-neogeo-rust] connecting gdb={gdb_host}:{gdb_port} emucap=127.0.0.1:{emucap_port} system={system}"
    );
    ...
    .with_context(|| format!("connect MAME Lua GDB stub at {gdb_host}:{gdb_port}"))?;
    let mut bridge = NeoGeoBridge::new(gdb, GdbBridgeEnv::from_process_env(), system)?;
    let dependency =
    ...
    .context("serve reconnecting Neo Geo session")
}
